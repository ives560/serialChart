    ...
    QAbstractTableModel(parent)
{
    ...
    for (int i = 0; i < m_rowCount; i++) {
        ...
        for (int k = 0; k < dataVec->size(); k++) {
            if (k % 2 == 0)
                dataVec->replace(k, i*2);
            else
                dataVec->replace(k, qrand() % 10);
        }
        m_data.append(dataVec);
    }
}

void CustomTableModel::addRow()
{
    int count = rowCount();
    insertRows(count,1);
    int k=0;
    for(k=0;k<m_columnCount;k++)
    {
...
bool CustomTableModel::insertRows(int row, int count, const QModelIndex &parent)
{
    ...
    for(int i=0;i<count;i++)
        m_data.insert(row+i,new QVector<qreal>(m_columnCount));
    endInsertRows();
    return true;
}
